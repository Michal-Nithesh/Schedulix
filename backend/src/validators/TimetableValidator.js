export class TimetableValidator {
  ...
  validate(entries = this.entries) {
    ...
    for (const entry of entries) {
      ...
      if (!division || !subject || !room || !slot) {
        conflicts.push(
          this.conflict(
            'INVALID_ASSIGNMENT',
            'Entry references missing division, subject, faculty, room, or time slot.',
            { entryIds: [entry.id], entry },
          ),
        );
        continue;
      }
      ...
      if (this.faculty.length > 0 && !faculty.qualifications?.includes(subject.id)) {
        conflicts.push(
          this.conflict('FACULTY_NOT_QUALIFIED', `Faculty ${faculty.name} is not qualified for ${subject.name}.`, {
      ...
      if (this.facultyAvailabilityMap[faculty.id]?.[entry.timeSlotId] === false) {
        conflicts.push(
          this.conflict('FACULTY_UNAVAILABLE', `Faculty ${faculty.name} is unavailable during this slot.`, {
            facultyId: faculty.id,
            timeSlotId: entry.timeSlotId,
            entryIds: [entry.id],
          }),
      ...
      if (room.roomType !== requirement.roomType) {
        conflicts.push(
          this.conflict(
            'ROOM_TYPE_MISMATCH',
            `Room ${room.name} does not satisfy the ${requirement.roomType} requirement.`,
            { roomId: room.id, subjectId: subject.id, timeSlotId: entry.timeSlotId, entryIds: [entry.id] },
          ),
        );
      }
      ...
      if (Number(room.capacity) < Math.max(Number(division.studentCount), requirement.requiredSeats)) {
        conflicts.push(
          this.conflict('ROOM_CAPACITY', `Room ${room.name} cannot fit ${division.studentCount} students.`, {
            roomId: room.id,
      ...
      if (this.roomAvailability[room.id]?.[entry.timeSlotId] === false) {
        conflicts.push(
          this.conflict('ROOM_UNAVAILABLE', `Room ${room.name} is unavailable during this slot.`, {
            roomId: room.id,
            timeSlotId: entry.timeSlotId,
            entryIds: [entry.id],
          }),
    ...
    this.addCollisionConflicts(
      conflicts,
      byDivision,
      'DIVISION_CONFLICT',
      'A division is assigned multiple subjects in the same slot.',
      'divisionId',
    );
    this.addCollisionConflicts(
      conflicts,
      byFaculty,
      'FACULTY_CONFLICT',
      'Faculty is assigned to multiple divisions in the same slot.',
      'facultyId',
    );
    this.addCollisionConflicts(
      conflicts,
      byRoom,
      'ROOM_CONFLICT',
      'A classroom is assigned to multiple divisions in the same slot.',
      'roomId',
    );
  ...
  getFaculty(facultyId) {
    return (
      this.faculty.find((member) => member.id === facultyId) || { id: facultyId, name: facultyId, qualifications: null }
    );
  }

  getSubjectRequirement(subjectId, divisionId) {
    const record = Array.isArray(this.subjectRequirementsMap)
      ? this.subjectRequirementsMap.find(
          (item) => item.subjectId === subjectId && (!item.divisionId || item.divisionId === divisionId),
        )
  ...
  getRequiredSessions(division, assignment) {
    ...
    const requirement = Array.isArray(this.subjectRequirementsMap)
      ? this.subjectRequirementsMap.find(
          (item) => item.subjectId === assignment.subjectId && item.divisionId === division.id,
        )
      : null;
        ...
        if (actualCount !== Number(expectedCount)) {
          conflicts.push(
            this.conflict(
              ...
              `Expected ${expectedCount} sessions for ${subjectId}, found ${actualCount}.`,
              { subjectId, expectedCount: Number(expectedCount), actualCount },
            ),
          );
        }
      }
    ...
    for (const division of this.divisions) {
      for (const assignment of this.getAssignmentsForDivision(division)) {
        ...
        const actualCount = entries.filter(
          (entry) => entry.divisionId === division.id && entry.subjectId === assignment.subjectId,
        ).length;
        if (actualCount !== expectedCount) {
          conflicts.push(
            this.conflict(
              'SESSION_COUNT_MISMATCH',
              `Expected ${expectedCount} sessions for ${assignment.subjectId} in ${division.name}, found ${actualCount}.`,
              { divisionId: division.id, subjectId: assignment.subjectId, expectedCount, actualCount },
            ),
          );
        }
      }
